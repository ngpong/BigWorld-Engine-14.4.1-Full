...
class _ParentAnchorWindow(object):
	...
	def handleMouseEnterEvent( self, comp ):
		#print "W1Script.handleMouseEnterEvent", comp
		self.component.hoverLabel.text = "!!"
		return False
		
	def handleMouseLeaveEvent( self, comp ):
		#print "W1Script.handleMouseLeaveEvent", comp
		self.component.hoverLabel.text = ""
		return False
...
class _FocusableComponent:
	...
	def handleKeyEvent( self, event ):
		...
		if event.key == Keys.KEY_SPACE:
			self.component.colour = (random.random()*255, random.random()*255, random.random()*255, 255)
			return True
		return False
...
class _MouseEventTester1: # Tests events on moving components.
	...
	def _animatePosition( self ):
		...
		if self.parent is not None and self.parent in GUI.roots() or \
		  self.component in GUI.roots():
			BigWorld.callback( 0.01, self._animatePosition )
...
class _MouseEventTester2: # Tests events when going visible/invisible.
	...
	def handleKeyEvent( self, event ):
		...
		return False # Allow other things to get key events
	...
	def _hideTimer( self ):
		if self.toggleFromHierarchy:		
			if self.parent is not None:
				...
				if self.component in GUI.roots():
