...
class MatchInfo:
	def __init__( self, tuple ):
		( self.trigger_minEntitySpeed,
		self.trigger_maxEntitySpeed,
		self.trigger_minEntityAux1,
		self.trigger_maxEntityAux1,
		self.trigger_minModelYaw,
		self.trigger_maxModelYaw,
		self.cancel_minEntitySpeed,
		self.cancel_maxEntitySpeed,
		self.cancel_minEntityAux1,
		self.cancel_maxEntityAux1,
		self.cancel_minModelYaw,
		self.cancel_maxModelYaw ) = tuple
...
class ActionDisplay:
	...
	def setAngle( self, g, deg ):
		...
		g.gen_shadow.uTransform_diffuse = ( m.get(0,0), m.get(1,0), 0, m.get(3,0) )
		g.gen_shadow.vTransform_diffuse = ( m.get(0,1), m.get(1,1), 0, m.get(3,1) )
		g.gen_shadow.uTransform = ( n.get(0,0), n.get(1,0), 0, n.get(3,0) )
		g.gen_shadow.vTransform = ( n.get(0,1), n.get(1,1), 0, n.get(3,1) )


	def setAngles( self, g, min, max ):
		#hemisphere angle gizmo can only do 180 degrees
		if (max - min) > 180:
			max = min + 180
	...
	def showActionInfo( self, entity, actionName ):
		...
		if mi.trigger_minEntitySpeed > -1000.0 or mi.trigger_maxEntitySpeed < 1000.0:
			...
			self.speedTrigger.component.position = ( 0, mi.trigger_minEntitySpeed, 0.0 )	#frame2s override their vertex z to always be 1 anyway.
		...
		if mi.cancel_minEntitySpeed > -1000 or mi.cancel_maxEntitySpeed < 1000.0:
			...
			self.speedCancel.component.position = ( 0, mi.cancel_minEntitySpeed, 0.0 )	#frame2s override their vertex z to always be 1 anyway.
