...
#This event just prints out one statement every 20% of the event,
#for debugging the timing of that event.
...
class FlickeringLight( Event ):
	...
	def go( self, effect, actor, source, target, **kargs ):
		...
		self.lightFaderAnim.keyframes = [(0.0,(0,0,0,0)),(0.25,col),(duration*0.5,col),(duration,(0,0,0,0))]
		...
		try: self.light.source = source.model.root
		except:
			try:	self.light.source = source.root
			except: self.light.source = source.node("Scene Root")
