...
class PPAnimateProperty( Event ):
	...
	def load( self, pSection, prereqs = None ):
		...
		'''
		#	TODO : when Vector4 are serialisable, make a generic light
		#	Animation event.
		#	TODO : allow the animation to be dependent on the duration of
		#	the reset of the effect.
		self.property = pSection.asString
		self.time = 0.0
		keys = []
		for (name,sect) in pSection.items():
			if name == "Key":
				self.time = sect.asFloat	#keep setting self.time, this will store the last time
		...
		self.animation.duration = 1000000.0	#somebody please give Math.Vector4Animation a looped property
		return self
