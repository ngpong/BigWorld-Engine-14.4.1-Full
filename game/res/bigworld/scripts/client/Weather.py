...
if BigWorld.component != "editor":
	import FantasyDemo as Personality	
else:
	import Personality	
...
class WeatherSystem:
	...
	def _onLoadSkyBoxes( self, callback, resourceRef ):
		self.loaded = True
		#Make sure we go through the list in the correct order;
		#the order in resourceRef is indeterminate (uses a map)
	...
	def _onLoadFX( self, callback, resourceRef ):
		...
		if self.loaded:
			...
			if BigWorld.component == "editor":
				...
				# node joints in the weather SFX get attached to the player model.
	...
	def loadEffects( self, fadeSpeed = 0.0 ):
		if self.bloom:
	...
	def fadeIn( self, fadingIn, fadeSpeed, immediate = False ):
		...
		if fadingIn:
			...
			try:
				...
				pass	#the space does not exist
			except EnvironmentError:
				pass	#the weather object does not exist
			self.loadEffects( fadeSpeed )
		else:
			if self.loaded:
	...
	def edReloadSkyBoxes( self ):
		...
		'''
		#Keep a temporary reference of the existing sky box models while
		#we rejig the list, so no resources get unloaded.
...
class Weather( Listenable ):
	...
	def loadEffects( self ):
		'''
		Add the weather system's effects to the current post processing chain.
		'''
		if self.system is not None:
			self.system.loadEffects()
	...
	def onChangeSpace( self, spaceID = -1, spaceSettings = None ):
		...
		if BigWorld.component != "editor":
			...
			try:
				...
				rds.lastWeatherSync[spaceID] = default
				weather = rds.lastWeatherSync[spaceID]
	...
	def summon( self, systemName, immediate = False, serverSync = False, resummon = False ):
		...
		for ds in weatherXML.values():
			if ds.name == systemName:
				#If the weather system has been activated by the player
				#and environment sync is on, then send this up to the server
				#so it can synchronise the weather for all other players that
				#are listening to server weather
				try:
					...
					#no player, or no getEnvironmentSync.  This is either
					#the editor, or the player has not been created yet.
					pass	
	...
	def _systemReadySummon( self, system, immediate, resummon ):
		...
		#if not resummon:
		#	Personality.addChatMsg( -1, 'Weather forecast : %s' % (system.name,) )
