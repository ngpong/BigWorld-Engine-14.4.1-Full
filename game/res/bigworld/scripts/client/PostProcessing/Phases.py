...
# Phase Factory
#
# All phase building functions that you want registered for use in WorldEditor
# must be decorated with @implementPhaseFactory( "fnName" )
#
#------------------------------------------------------------------------------
s_phaseFactories = {}
...
# Section - Phases
#
# generic phases used by multiple Effects
#------------------------------------------------------------------------------
...
def buildBackBufferCopyPhase( renderTarget ):
	...
	if renderTarget == None:
		...
		renderTarget = rt("backBufferCopy")
...
def buildPhase( input, output, fxFile, sampleProvider = straightTransfer4Tap, srcBlend = BW_BLEND_ONE, destBlend = BW_BLEND_ZERO ):
	if input == None:
		from RenderTargets import rt
		input = rt("backBufferCopy").texture
...
def buildColourScalePhase( input, output, srcBlend = BW_BLEND_ONE, destBlend = BW_BLEND_ZERO ):
	p = buildPhase( input, output, "shaders/post_processing/legacy/colour_scale.fx", straightTransfer4Tap, srcBlend, destBlend )
	p.name = "colour scale"
	return p
...
def buildBlurPhase( input, output, horiz, filterMode = 0, width = 1.0 ):
	...
	p = buildPhase( input, output, "shaders/post_processing/legacy/gaussian_blur.fx", sampleProv, BW_BLEND_ONE, BW_BLEND_ONE )
...
def buildNTapFilterPhase( input, output, filter, srcBlend = BW_BLEND_ONE, destBlend = BW_BLEND_ZERO ):
	...
	for (u,v,weight) in taps:
		setattr( p.material, "tap%d" % (idx,), (u,v,weight,0) )
		idx += 1
	return p
...
def build9TapFilterPhase( input, output, filter, srcBlend = BW_BLEND_SRCALPHA, destBlend = BW_BLEND_INVSRCALPHA ):
	...
	p.name = "9-tap filter"
	taps = filter()
	p.material.tap1 = ( taps[0][0], taps[0][1], taps[0][2], 0 )
	p.material.tap2 = ( taps[1][0], taps[1][1], taps[1][2], 0 )
	p.material.tap3 = ( taps[2][0], taps[2][1], taps[2][2], 0 )
	p.material.tap4 = ( taps[3][0], taps[3][1], taps[3][2], 0 )
	p.material.tap5 = ( taps[4][0], taps[4][1], taps[4][2], 0 )
	p.material.tap6 = ( taps[5][0], taps[5][1], taps[5][2], 0 )
	p.material.tap7 = ( taps[6][0], taps[6][1], taps[6][2], 0 )
	p.material.tap8 = ( taps[7][0], taps[7][1], taps[7][2], 0 )
	p.material.tap9 = ( taps[8][0], taps[8][1], taps[8][2], 0 )
	return p
