...
def linkMaterialAndBypass(effect, material, attributeName):
	...
	try:		
		v4Morph = Math.Vector4Morph((val,val,val,val))
	except TypeError:
		val = val.target[3]
		v4Morph = Math.Vector4Morph((val,val,val,val))
...
class BypassProperty:
	...
	def set( self, amt, speed = 0.01 ):
		...
		try:
			e.bypass.target = (amt,amt,amt,amt)
	...
	def onSetChain(self):
		...
		if e.bypass != None:
			try:
				...
				e.bypass = Math.Vector4Morph( (val,val,val,val) )
			except TypeError:
...
class MaterialProperty:
	...
	def onSetChain(self):
		try:
			...
			return
		p = e.phases[self.phaseIdx]
		#linkMaterialAndBypass( e, p.material, self.materialAttribute )
