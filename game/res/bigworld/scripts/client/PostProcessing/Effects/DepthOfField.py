...
#def setFoV( focalLen, speed = 0.01 ):
#	if focalLen < 0.001:
#		return
#	filmDiagonal = math.sqrt(0.035 * 0.035)
#	fovRad = 2.0 * math.atan(filmDiagonal / (2 * focalLen))
#	fovRad /= 1.25
#	#print "************ setFoV", focalLen, fovRad
#	BigWorld.projection().rampFov(fovRad, speed)


#def formatFocalLength(val):
#	return "%0.0fmm" % (val * 1000.0,)
		

#def formatAperture(val):
#	fLen = getFocalLength()
#	if val < 0.001:
#		val = fLen * 56.0
#	if val > 0.001:
#		return "f/%0.1f" % (fLen/val,)
#	else:
#		return "0.0"
...
def lensSimulation():
	...
	fpComputeBuffer1 = rt("fpComputeBuffer1")
...
def lensExplicit():
	...
	#debug view the floating point buffer
	#w.renderTarget = backBufferCopy
	e = Effect()
...
def depthOfFieldBokehControl():
	...
	backBufferCopy = rt("backBufferCopy")
	fpComputeBuffer1 = rt("fpComputeBuffer1")
	bkComputeBuffer = rt("bkComputeBuffer")
