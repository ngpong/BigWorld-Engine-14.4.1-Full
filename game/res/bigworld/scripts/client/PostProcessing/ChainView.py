...
class EffectView:
	...
	def __init__( self, effect, eIdx, maxPhases ):
		...
		effectWindow = GUI.Window()
		self.setToClipMode( effectWindow )
		effectWindow.size = (2,0.05)
		effectWindow.position = (0,0.95,1)
		effectWindow.verticalAnchor = "BOTTOM"
		effectWindow.textureName = "system/maps/col_white.dds"
		effectWindow.colour = (0,92,0,92)
		effectWindow.materialFX = "BLEND"
		
		self.addLabel( effectWindow, effect.name )
		if effect.bypass != None:
			self.addGraph( effectWindow, effect.bypass )
		
		phasesWindow = GUI.Window()
		self.setToClipMode( phasesWindow )
		phasesWindow.size = (2,1.95)
		phasesWindow.position = (0,0.95,1)
		phasesWindow.verticalAnchor = "TOP"
		...
		for p in effect.phases:
			...
			phasesWindow.addChild(s)
			pIdx += 1
			
		self.window.effect = effectWindow
		self.window.phases = phasesWindow

	def drop( self ):

		# Effect window
		if self.window.effect.parent is not None:
			self.window.effect.parent.delChild( self.window.effect )
		else:
			GUI.delRoot( self.window.effect )
		self.window.effect = None

		# Phases
		# Delete children
		for (name, c) in self.window.phases:
			self.window.phases.delChild( c )

		if self.window.phases.parent is not None:
			self.window.phases.parent.delChild( self.window.phases )
		else:
			GUI.delRoot( self.window.phases )
		self.window.phases = None
		...
		if self.window.parent is not None:
			self.window.parent.delChild( self.window )
		else:
	...
	def createPhase( self,eIdx,pIdx,e,p,maxPhases ):
		...
		#uvs returned as ( l, b, r, t )
		uvs = d.phaseUV( eIdx, pIdx, len(effects), len(e.phases) )
		bl = (uvs[0],1.0 - uvs[1])
		tl = (uvs[0],1.0 - uvs[3])
		tr = (uvs[2],1.0 - uvs[3])
		br = (uvs[2],1.0 - uvs[1])
		#mapping attribute wants tl, bl, br, tr
...
class ChainView:
	...
	def calcScrollBounds( self ):
		...
		#Allow dragging 95% of the gui off-screen, but not all.
		xs[0] -= 0.95
		ys[0] -= 0.95
		xs[1] += 0.95
		ys[1] += 0.95
		...
		#self.scrollTo( self, self.scroll[0], self.scroll[1] )
