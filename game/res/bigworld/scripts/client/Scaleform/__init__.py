...
try:
	...
	print "There is no module named _Scaleform.  The most likely cause of this \
	is the client was built without Scaleform support.  Please check the \
	documentation for further details on enabling Scaleform support."


#------------------------------------------------------------------------------
# Section - test and example functions
#------------------------------------------------------------------------------
def showCursor():
...
def createMovieGUI(  file = "scaleform/d3d9guide.swf" ):
	...
	import GUI
	for i in GUI.roots(): i.position[2] = max(i.position[2],0.1)
	f = GUI.Flash(m)
	f.position = (0,0,0)
	f.focus = True
	f.moveFocus = True
...
def createIME(  ):
	...
	#mapFont( "$IMECandidateListFont", "KozGoPro-Bold" )
	createIMEFontsMovie()
	showCursor()
...
def createFontMovie( file = "scaleform/drawtext_fonts.swf" ):
	...
	d.setAsFontMovie()
	d.addToFontLibrary()
	#dont return m,d.  Let the python instances die.
	#because the setAtFontMovie call registers the font
	#movie with the Scaleform loader object, that object
	#now owns its existence.
	#return m,d
...
#Make sure you create the font movie first
