...
float4	 factor : FontColour;
float4	 resolution : Screen;	//w,h,halfw,halfh
bool pixelSnapEnabled : FontPixelSnap;

// Manual variables
texture diffuseMap : FontMap;
float uvDivisor : FontUVDivisor;
float4  diffuseColour = (1,1,1,1);
int     filterType = 1; // point filtering
...
OutputDiffuseLighting vs_main( VertexXYZDUV2 input, uniform bool pixelSnap )
{
	...
	float2 tcFactor = {1.0, 1.0/uvDivisor};
	o.tc = input.tc * tcFactor;
	o.tc2 = input.tc2 * tcFactor;
	o.diffuse = diffuseColour * input.diffuse;
