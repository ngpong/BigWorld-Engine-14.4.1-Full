...
class _BWCoroutineThread:
	...
	def _tick( self ):
		...
		self._advanceGenerator( False )

	######
	#
	# This function either does next on the generator or throws an exception on the generator
	# In any case the handling of the return value should be the same.
	#
	def _advanceGenerator( self, shouldThrow ):
		try:
			if shouldThrow:
...
class BWWaitForCondition( _BWWaitObject ):
	'''This class implements a wait object that when yielded, poles its
	...
	def __init__( self, condition, timeout = None, checkFrequency = 0.01 ):
		...
		if timeout is not None:
			self._timeoutTime = time.clock() + timeout
		else:
	...
	def _tick( self ):
		...
		elif self._timeoutTime is not None and time.clock() >= self._timeoutTime:
...
class BWWaitForCoroutine( _BWWaitObject ):
	...
	def __init__( self, waitThread, timeout = None ):
		...
		self._waitThread._completionCallback = self.handleCompletionCallback	# todo: remove use of private
...
def _niceFunctionString( f ):
	assert callable( f ) or isinstance( f, types.GeneratorType ) or isinstance( f, _BWCoroutineFunction)

	if isinstance( f, partial ):
		return "partial %s %s %s:%d" % ( f.func.func_name, f.args, f.func.func_code.co_filename, f.func.func_code.co_firstlineno )
