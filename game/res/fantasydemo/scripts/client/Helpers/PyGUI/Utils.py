...
def applyMapping( component, mappingType, mapping ):
	if mappingType == 'PIXEL':
		...
		if texture:
			clipMapping = [None, None, None, None]
...
def mouseScreenPosition():
	...
	return ( (clipPos[0] + 1.)/2. * res[0], (-clipPos[1] + 1.)/2. * res[1] )
...
# -------------------------------------------------------------------------
# This class is a helper for gui structures requiring a dynamic number of
# elements.
# -------------------------------------------------------------------------
class Budget:
...
# ------------------------------------------------------------------------------
# This class is a simple grid layout manager.
# This class assumes the parent control is not a window
# This class assumes the child controls are all the same
# size, and are centered horizontally and vertically
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Section: BlinkingCursor - a re-usable blinking-cursor (edit fields, etc)
# ------------------------------------------------------------------------------
CURSOR_WIDTH 		= 1 		# Width in pixels (real pixels, not res overridden pixels)
CURSOR_BLINK_PERIOD	= 0.75		# Time in seconds for a full blink period (time taken to hide and show again) TODO: get OS blink speeed
CURSOR_COLOUR 		= (255,255,255,255)

class BlinkingCursor( PyGUIBase ):

	@staticmethod
	def create():
		component = GUI.Simple( "system/maps/col_white.bmp" )
		component.materialFX = "BLEND"
		component.widthMode = "PIXEL"
		component.heightMode = "CLIP"
		component.width = CURSOR_WIDTH * getHPixelScalar()
		component.height = 1.5
		component.horizontalPositionMode = "PIXEL"
		component.horizontalAnchor = "LEFT"
		component.position.x = 0
		component.position.z = 0.25 # Bring forward from default Z
		
		component.blinker = GUI.ColourShader()
		component.blinker.colourProvider = blinkingColourProvider( CURSOR_COLOUR, CURSOR_BLINK_PERIOD )

		component.visibleInAppFocusOnly = True;
		
		component.script = BlinkingCursor( component )
		return component
	...
	def enable( self, enable ):
		self.component.visible = enable
		
	def touch( self ):
		self.component.blinker.colourProvider.a.time = CURSOR_BLINK_PERIOD / 2.0
		self.component.width = CURSOR_WIDTH * getHPixelScalar()
		
	def getScreenClipPosition( self, bottomLeft = False ):	
		y = -self.component.height/2.0 if bottomLeft else 0
		return self.component.localToScreen( (0, y) )
		
	def clipBounds( self ):
		return clipRegion( self.component )
		

#This function used for communication with flash
#decodes the string from the array
def fromUTF8Array (array):
	return "".join(["%s" % chr(el) for el in array])

#This function used for communication with flash
#encodes the array to be sent to flash from a string
