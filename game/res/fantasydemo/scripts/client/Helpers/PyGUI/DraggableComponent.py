...
class DraggableComponent:
	...
	def startDragging( self, cursorPosition ):
		dragManager.startDragging( self.component, self.horzDrag,
									self.vertDrag, self.restrictToParent,
									cursorPosition )
...
class ComponentDragManager:
	...
	def startDragging( self, component, horzDrag, vertDrag, restrictToParent, mpos ):
		...
		#print "starting to drag component", component, cpos, mpos, self.offset
	...
	def stopDragging( self ):
		...
		#print "stopping component drag", self.component
		self.component = None


	def handleKeyEvent( self, event ):
		if not event.isRepeatedEvent() and \
		  event.key == Keys.KEY_LEFTMOUSE and not event.isKeyDown() and self.isDragging():
			self.stopDragging()
			return True
	...
	def handleMouseEvent( self, event ):
		...
		# Desired position in clip space
		cpos = Math.Vector2( mpos[0] - self.offset.x, mpos[1] - self.offset.y )

		# Work out the desired position in whatever local space the
		# component is in (ie. if it is in a WindowGUIComponent).
		...
		#print "moving dragged component", dx, dy, mpos[0], mpos[1]

		# Actually move the component. Restrict it as neccessary.
