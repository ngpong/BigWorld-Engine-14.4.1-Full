...
class ToolTipInfo( object ):
	...
	def __init__( self, component = None, templateName = None, infoDictionary = {}, infoArea = None,
					delayType = None, placement = None ):
		if templateName:
...
class ToolTipManager( object ):
	...
	def fini( self ):
		ToolTipManager.instance.toolTipGUIs = {}
		ToolTipManager.instance = None
	...
	def setupToolTip( self, component, toolTipInfo ):
		...
		# Just in case the previous tool tip hadn't been removed from the screen.
		if self.toolTipGUI != self.toolTipGUIs[ toolTipInfo.templateName ] and \
		  self.toolTipGUI is not None and \
		  self.rootGUI.hasChild( self.toolTipGUI, False ):
	...
	def handleMouseEvent( self, event ):
		if self.showToolTipCallback or self.toolTipGUI:
			...
			if mpos.x < self.infoAreaLeft or mpos.x > self.infoAreaRight or mpos.y < self.infoAreaBottom or mpos.y > self.infoAreaTop:
				# Remove tool tip or cancel callback
				if self.toolTipGUI:
					self.rootGUI.delChild( self.toolTipGUI )
					self.toolTipGUI = None
	...
	def _showToolTip( self, component, toolTipInfo ):
		...
		self.showToolTipCallback = None
		infoArea = (self.infoAreaLeft, self.infoAreaTop, self.infoAreaRight, self.infoAreaBottom)
		self.toolTipGUI.script.setupToolTip( toolTipInfo, infoArea )
		if not self.rootGUI.hasChild( self.toolTipGUI, False ):
