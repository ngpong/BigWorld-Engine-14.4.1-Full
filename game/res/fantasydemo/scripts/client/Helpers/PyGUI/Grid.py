...
class Grid( PyGUIBase ):
	...
	def doLayout( self ):
		...
		if widthMode == 'PIXEL':
			horizSize = (self.component.width - self.borderLeft - self.borderRight \
							- self.horizontalGap * (self.gridWidth - 1)) / self.gridWidth
			horizOffset = horizSize + self.horizontalGap
			horizStart = self.borderLeft
		if widthMode == 'CLIP':
			horizSize = (2.0 - self.borderLeft - self.borderRight \
							- self.horizontalGap * (self.gridWidth - 1)) / self.gridWidth
			horizOffset = horizSize + self.horizontalGap
			horizStart = -1.0 + self.borderLeft

		if heightMode == 'PIXEL':
			vertSize = (self.component.height - self.borderTop - self.borderBottom \
							- self.verticalGap * (self.gridHeight - 1)) / self.gridHeight
			vertOffset = vertSize + self.verticalGap
			vertStart = self.borderTop
		if heightMode == 'CLIP':
			vertSize = (2.0 - self.borderTop - self.borderBottom \
							- self.verticalGap * (self.gridHeight - 1)) / self.gridHeight
			vertOffset = -(vertSize + self.verticalGap)
