...
import math
from FX.Events.ParticleSubSystem import ParticleSubSystem
from FX.Events.ParticleSubSystem import MATRIX_SWARM_PSA
...
#------------------------------------------------------------------------------
#	AOEVictimNodeList - this event finds all victims in the area of effect,
#	and sets the node list into the actors' MatrixSwarmPSA target lists
#------------------------------------------------------------------------------
class AOEVictimNodeList( ParticleSubSystem ):
	'''
	This class implements an Event that finds all victims in the area of effect
	and sets the node list into the actor's MatrixSwarmPSA target list.
	...
	#we are only interested in sub-systems that have Matrix Swarm PSAs.
	...
	#--------------------------------------------------------------------------------
	# This method returns a matrix provider.
	# It creates a random vector on the designated side of the origin, and collides
	# it with the scene ( or maxRange. )
	#--------------------------------------------------------------------------------
	...
	def findTargets( self, actor, source, target ):
		...
		#get the vector heading off to the right
		...
		if hasattr( self, "victims" ):
			...
			for i in team.members.keys():
				...
				if entity:
					#print entity, entity.position, origin, self.maxRange
