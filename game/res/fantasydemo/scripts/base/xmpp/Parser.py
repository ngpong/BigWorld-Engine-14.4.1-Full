...
class XMPPParser( object ):
	...
	def __init__( self ):
		self.stanzaStream = [] # stream stanzas
		self.stanzaStack = [] # incomplete stanzas (excluding main stream stanza)
		self.stanzaComplete = [] # complete stanzas
	...
	## The following methods are called from the XML Parser to allow us to
	## handle any specific XMPP parsing

	def startElement( self, name, attr ):
		if name == 'stream:stream':
			### check that start element is stream:stream or error
			self.stanzaStream.append(
				{ 'name': name, 'attr': attr, 'data': "", 'children': [] } )

		else:
			self.stanzaStack.append(
				{ 'name': name, 'attr': attr, 'data': "", 'children': [] } )
	...
	def endElement( self, name ):
		if len( self.stanzaStack ) > 1:
			...
			### check that end element is /stream:stream or error
