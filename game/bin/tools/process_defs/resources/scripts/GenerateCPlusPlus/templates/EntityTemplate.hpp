...
{% if hasBaseMailbox %}#include "{{ entityMailBoxPath }}/{{ entity.name }}_BaseMB.hpp"{% endif %}
{% if hasCellMailbox %}#include "{{ entityMailBoxPath }}/{{ entity.name }}_CellMB.hpp"{% endif %}
...
{%- endfor %}
	...
	// Mailboxes (if any)
{% if hasCellMailbox %}	const CellMB::{{ entity.name }} & cell() const 
	{
		return cell_;
	}
{% endif %}
...
{% if hasCellMailbox %}	CellMB::{{ entity.name }} cell_;{% endif %}
{% if hasBaseMailbox %}	BaseMB::{{ entity.name }} base_;{% endif %}
