macroScript Standard_Portal
	category:"BigWorld"
	toolTip:"Add Standard Portal"
...
(
	...
	(
		...
		if findString selected.name "exit_portal" != undefined do -- already has exit_portal suffix
		(
			originalName = selected.name
			portalSuffixPos = findString selected.name "exit_portal"
			newName = replace originalName portalSuffixPos  11 ""
			selected.name = newName
		)
		
		if findString selected.name "heaven_portal" != undefined do -- already has heaven_portal suffix
		(
			originalName = selected.name
		...
		selected.name = selected.name + "_portal"
		
		selected.material = currentMaterialLibrary["standard_portal"]
		selected.wirecolor = color 0 0 255
	...
	(
		...
		(
			...
			(				
				...
				if faceMaterial == "standard_portal" then
				(
				...
				(
					...
					$.material = currentMaterialLibrary["standard_portal"]
				)
			)
			else
			(
	...
	(
		...
		(
			...
			if stdPort == true and heavPort == false and exPort == false then -- is standard portal
			(
				...
				portalSuffixPos = findString selection[i].name "_portal"
