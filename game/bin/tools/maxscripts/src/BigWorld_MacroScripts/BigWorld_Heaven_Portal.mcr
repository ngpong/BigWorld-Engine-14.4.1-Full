macroScript Heaven_Portal
	category:"BigWorld"
	toolTip:"Add Heaven Portal"
	Icon:#("bigworld_icons", 3)
...
(
	...
	(
		...
		if findString selected.name "exit_portal" != undefined do -- already has exit_portal suffix, must test this first before _portal or fails
		(
		...
		setUserProp selected "heaven" "true"
		setUserProp selected "exit" "false"
		
		-- new for collada, removing suffix _portal				
		selected.name = selected.name + "heaven_portal"
		
		selected.material = currentMaterialLibrary["heaven_portal"]
		selected.wirecolor = color 0 255 0		
	)
	...
	(
		...
		(
			...
			(
				...
				if faceMaterial == "heaven_portal" then
				(
				...
				(
					...
					$.material = currentMaterialLibrary["heaven_portal"]
				)
			)
			else
			(
	...
	(
		...
		(
			...
			if stdPort == true and heavPort == true and exPort == false then -- is heaven portal
			(
				...
				portalSuffixPos = findString selection[i].name "heaven_portal"
