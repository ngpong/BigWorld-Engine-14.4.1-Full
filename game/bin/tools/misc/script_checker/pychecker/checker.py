...
class Module :
    ...
    def filename(self) :
        if hasattr( self, "exception_filename" ):
            return self.exception_filename
    ...
    def load(self) :
        try :
            ...
            self.exception_filename = e.filename
...
def main(argv) :
    ...
    for moduleName in _getModules(files) :
        ...
        if not module.load() :
            w = warn.Warning(module.filename(), module.lineno,
                                "NOT PROCESSED UNABLE TO IMPORT", "error:")
