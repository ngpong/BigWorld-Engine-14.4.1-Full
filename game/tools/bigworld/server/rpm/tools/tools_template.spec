...
Requires: bigworld-bwmachined, epel-release, TurboGears < 2.0, MySQL-python >= 1.2.2, python-ldap, python-simplejson
## Obsoletes:
## Conflicts:
...
if test -f %{_initrddir}/%{service_bwlockd}; then
	/sbin/service %{service_bwlockd} stop > /dev/null 2>&1
	/sbin/chkconfig --del %{service_bwlockd}
fi
...
su -c "/sbin/service bw_stat_logger loginit" > /dev/null 2>&1 || true
...
if [ "$1" -eq 0 ]; then
	/sbin/service bwlockd stop > /dev/null 2>&1
	/sbin/chkconfig --del bwlockd
fi
