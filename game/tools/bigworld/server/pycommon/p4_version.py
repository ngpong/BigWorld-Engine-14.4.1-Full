...
def loadVersionInfo():
	...
	serverAddressResult = re.search( "Server address: (\S+)", cmdOutput )
	try:
		repoLocation = serverAddressResult.groups()[0]
	except:
		# p4 not behaving normally.
		log.warning( "Unable to get server address from 'p4 info'." )
		return None


	# Ensure we are within a workspace (this command will raise exceptions if
	# we are not in a workspace directory).
	command = ["p4", "where" ]
	cmdOutput = runConnectedP4Command( command )
	if not cmdOutput:
		return None
	...
	remoteRev = getP4Changes()

	return { "revisionNumber": str( localRev ),
			 "remoteRevisionNumber": str( remoteRev ),
			 "repositoryLocation": repoLocation
	}

	# if this point has been reached, then the load has been successful
# load
